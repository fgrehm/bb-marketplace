...
export function RoundupCard({ items, onOpen }: { items: Item[]; onOpen: (item: Item) => void }) {
  ...
  return <section className="overflow-hidden rounded-3xl bg-gradient-to-br from-amber-400/[0.07] via-card/70 to-card/40 shadow-sm ring-1 ring-border/50">
    ...
    <div className="px-4 pb-1"><p className="text-xl font-medium tracking-tight">Nothing is asking for attention.</p><p className="mt-2 text-sm text-muted-foreground">A few items resting in the real hoard. No recommendations or automatic decisions have been made.</p></div>
