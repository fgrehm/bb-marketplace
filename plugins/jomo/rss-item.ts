...
export function toRssItem(row: RssReviewRow): Item {
  return toHoardItem({ ...row, sourceId: row.sourceId ?? null, sourceColor: colorFromString(row.source), kind: row.kind, tags: "[]", state: "new", contentState: "staged", expiresAt: null });
}
