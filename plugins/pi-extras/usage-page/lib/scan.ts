...
async function readTranscriptRecords(
  ...
  try {
    ...
    for await (const line of lines) {
      ...
      const record = parsePiLine(line, sessionIdFallback);
...
export class UsageScanner {
  ...
  private async readSummaryUnlocked(input: ReadSummaryOptions): Promise<{
    ...
    const piDirs = await resolvePiDirs();

    const piFiles = await this.collectFiles(
      piDirs,
      sinceMs,
      cover.sinceDay,
      cover.untilDay,
    ...
    const pi = await this.recordsForFiles(
      "pi",
      piFiles.files,
      cover.sinceDay,
      cover.untilDay,
    ...
    const walkedRoots = [...piFiles.roots];
    ...
    const fileHits = pi.stats.fileHits;
    const fileMisses = pi.stats.fileMisses;
    const filesParsed = pi.stats.filesParsed;
